...
from src.core.paths import icon_file
...
from src.ui.eval_graph import EvalGraph
...
from src.ui.lines_panel import LinesPanel
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # Posée ici et non seulement dans run() : la fenêtre doit porter son
        # icône quel que soit le point d'entrée.
        icone = charger_icone()
        if not icone.isNull():
        ...
        self._fen_affichee = chess.Board().fen()
    ...
    def _build_ui(self) -> None:
        ...
        self.eval_graph.ply_selected.connect(self._goto)
        ...
        layout_gauche.setContentsMargins(10, 10, 6, 10)
        layout_gauche.setSpacing(8)
        layout_gauche.addLayout(colonne_plateau, stretch=1)

        carte_graphe, layout_graphe = carte()
        layout_graphe.setContentsMargins(0, 0, 0, 0)
        layout_graphe.addWidget(self.eval_graph)
        layout_gauche.addWidget(carte_graphe)

        layout_gauche.addLayout(nav)

        milieu, droite = self._build_panneaux()

        splitter = QSplitter(Qt.Orientation.Horizontal)
        splitter.addWidget(gauche)
        splitter.addWidget(milieu)
        splitter.addWidget(droite)
        splitter.setStretchFactor(0, 10)
        splitter.setStretchFactor(1, 2)
        splitter.setStretchFactor(2, 2)
        splitter.setChildrenCollapsible(False)
        # Répartition de départ : l'échiquier d'abord, le reste au minimum
        # utile. L'utilisateur reste libre de déplacer les poignées.
        splitter.setSizes([820, 250, 280])
    ...
    def _build_toolbar(self) -> None:
        ...
        barre.addSeparator()
        self.action_analyser = action(
            "Analyser",
            self._analyser,
            "Ctrl+R",
            "Réutilise l'analyse enregistrée si elle est assez profonde",
        )
        self.action_reanalyser = action(
            "Réanalyser",
            lambda: self._analyser(force=True),
            "Ctrl+Shift+R",
            "Refait l'analyse même si une version enregistrée existe",
        )
    ...
    def _build_panneaux(self) -> tuple[QWidget, QWidget]:
        ...
        layout_precision.addWidget(self.summary)

        variantes, layout_variantes = carte("Meilleures suites")
        self.lines_panel = LinesPanel()
        layout_variantes.addWidget(self.lines_panel)
        self.carte_variantes = variantes
        ...
        self.label_detail.setMinimumHeight(64)
        ...
        # Colonne du milieu : identité de la partie et bilan chiffré. Ces deux
        # blocs se lisent une fois, au contraire de la feuille de partie qu'on
        # parcourt en continu — d'où la séparation.
        milieu = QWidget()
        milieu.setMinimumWidth(210)
        layout_milieu = QVBoxLayout(milieu)
        layout_milieu.setContentsMargins(6, 10, 6, 10)
        layout_milieu.setSpacing(10)
        layout_milieu.addWidget(entete)
        layout_milieu.addWidget(precision)
        layout_milieu.addWidget(self.carte_variantes)
        layout_milieu.addStretch()

        # Colonne de droite : la lecture coup par coup.
        droite = QWidget()
        droite.setMinimumWidth(230)
    ...
    def _appliquer_reglages(self) -> None:
        ...
        palette = None
        if app is not None:
            palette = apply_theme(app, s.theme_mode)
        if palette is not None:
            self.eval_graph.set_colors(
                clair="#F0F0F0", sombre=palette.surface_haute, curseur=palette.accent
            )

        self.board_widget.set_theme(s.board_theme)
        self.board_widget.set_piece_set(s.piece_set)
        self.captures_haut.set_piece_set(s.piece_set)
        self.captures_bas.set_piece_set(s.piece_set)
        self.liste_coups.set_piece_set(s.piece_set)
    ...
    @staticmethod
    def _cle_pgn(pgn: str) -> str:
        """Identifiant stable pour une partie sans URL d'origine.

        Sans clé, chaque ouverture d'un même fichier créerait une ligne de
        plus et l'analyse précédente resterait introuvable. L'empreinte du PGN
        réutilise la colonne source_url, déjà unique : pas de migration de
        schéma pour une base existante.
        """
        return "pgn:" + hashlib.sha1(pgn.strip().encode("utf-8")).hexdigest()
    ...
    def _sauver_partie(self, game: chess.pgn.Game, pgn: str, **extra) -> None:
        ...
        if self._depot is None or not self._settings.autosave:
            return
        extra.setdefault("source_url", self._cle_pgn(pgn))
    ...
    def _signature_regles(self) -> str:
        """Identifie le jeu de règles ayant produit une analyse.

        Le réglage des badges ajoutés en fait partie : l'activer ou le couper
        change les verdicts, une analyse enregistrée sous l'autre réglage ne
        doit donc pas être réaffichée telle quelle.
        """
        suffixe = "+" if self._settings.detect_special else "-"
        return f"Stockfish · règles v{RULES_VERSION}{suffixe}"

    def _regles_a_jour(self, meta: dict) -> bool:
        """L'analyse enregistrée a-t-elle été produite par les règles actuelles ?

        Sans ce contrôle, une correction de la classification ne se voit jamais
        sur les parties déjà en base : le rapport périmé est simplement
        réaffiché, et le badge erroné semble persister malgré le correctif.
        """
        return meta.get("engine", "") == self._signature_regles()

    def _restaurer_analyse(self, silencieux: bool = False) -> bool:
        """Réapplique l'analyse stockée pour la partie courante, si elle existe."""
        if self._depot is None or self._game_id is None:
            return False
        try:
            rapport = self._depot.latest_report(self._game_id)
            meta = self._depot.analysis_meta(self._game_id)
        except DatabaseError:
            return False
        if rapport is None or meta is None:
            return False
        if not self._regles_a_jour(meta):
            if not silencieux:
                self.statusBar().showMessage(
                    "Analyse enregistrée obsolète — relance l'analyse (Ctrl+R).",
                    8000,
                )
            return False
        ...
        if not silencieux:
            date = meta["created_at"].strftime("%d/%m/%Y")
            self.statusBar().showMessage(
                f"Analyse du {date} réutilisée (profondeur {meta['depth']}). "
                "Ctrl+Maj+R pour refaire l'analyse.",
                10000,
            )
        return True
    ...
    def load_pgn(self, texte: str, persist: bool = True) -> bool:
        ...
        self.summary.clear()
        self.eval_graph.set_report(None)
        ...
        if persist:
            ...
            self._restaurer_analyse(silencieux=True)
        return True
    ...
    def _appliquer_rapport(self, report) -> None:
        ...
        self.eval_graph.set_report(report)
    ...
    def _goto(self, ply: int) -> None:
        ...
        if coup is None:
            ...
            self.eval_bar.set_evaluation(
                coup.analysis.after.cp_white,
                coup.analysis.after.mate_white,
                coup.analysis.after.outcome_white,
            )

        self.liste_coups.select_ply(ply)
        self.eval_graph.set_ply(ply)
        self._fen_affichee = self._navigator.board.fen()
        self.lines_panel.clear()
        self._live.request(self._navigator.board)
    ...
    def _maj_detail(self, coup) -> None:
        ...
        if classification is not MoveClass.BOOK:
            if not analyse.is_best and analyse.best_san:
                lignes.append(
                    f"Le moteur préférait <b>{to_figurine(analyse.best_san)}</b>."
                )
            issue = analyse.outcome_white
            if issue is not None:
                lignes.append(
                    {
                        1.0: "Les blancs gagnent.",
                        0.0: "Les noirs gagnent.",
                    }.get(issue, "Partie nulle.")
                )
            elif (mat := analyse.mate_for_mover) is not None:
    ...
    def _afficher_exploration(self, dernier: chess.Move | None, animer: bool) -> None:
        ...
        self._fen_affichee = self._explore_board.fen()
    ...
    def _juger_coup_explore(
        ...
        verdict = (
            f"<span style='color:{couleur};font-size:15px;font-weight:700'>"
            f"{classification.symbol or '•'} {to_figurine(analyse.san)} — "
        ...
        if not analyse.is_best and eval_avant.best_san:
            verdict += (
                ...
                f"<b>{to_figurine(eval_avant.best_san)}</b>"
                f"<span style='color:#8D94A0'>  ·  perte "
                f"{analyse.centipawn_loss} cp</span>"
            )
    ...
    def _on_live_result(self, evaluation: LiveEval) -> None:
        """Résultat du moteur continu.

        Les variantes s'affichent dans tous les cas ; le reste — jauge,
        verdict — n'a de sens qu'en exploration, la partie analysée ayant ses
        propres valeurs, plus profondes.
        """
        if evaluation.fen != self._fen_affichee:
            return  # résultat d'une position qu'on a déjà quittée

        self.lines_panel.set_lines(evaluation.lines)
    ...
    def _analyser(self, force: bool = False) -> None:
        if self._controller.running or not len(self._navigator):
            return

        # Une analyse déjà en base et au moins aussi profonde n'a aucune raison
        # d'être refaite : le résultat serait identique, pour plusieurs
        # dizaines de secondes de calcul.
        if not force and self._depot is not None and self._game_id is not None:
            try:
                meta = self._depot.analysis_meta(self._game_id)
            except DatabaseError:
                meta = None
            if (
                meta is not None
                and meta["depth"] >= self._settings.depth
                and self._regles_a_jour(meta)
            ):
                self._restaurer_analyse()
                return
        ...
        worker = AnalysisWorker(
            ...
            classifier_config=ClassifierConfig(detect_special=s.detect_special),
        ...
        if self._depot is not None and self._game_id is not None and self._settings.autosave:
            try:
                self._depot.save_analysis(
                    ...
                    book_plies=self._book_plies,
                    engine_name=self._signature_regles(),
                )
                message += " · enregistrée"
            except DatabaseError as exc:
...
def charger_icone() -> QIcon:
    """Icône de l'application, toutes tailles disponibles.

    Plusieurs résolutions plutôt qu'une seule : la barre des tâches et le
    sélecteur de fenêtres ne demandent pas la même, et un redimensionnement
    depuis une grande image donne un résultat flou à 16 pixels.
    """
    icone = QIcon()
    for taille in (16, 24, 32, 48, 64, 128, 256, 512):
        chemin = icon_file(taille)
        if chemin.is_file():
            icone.addFile(str(chemin))
    if icone.isNull():
        svg = icon_file()
        if svg.is_file():
            icone.addFile(str(svg))
    return icone
...
def run(pgn_path: str | None = None) -> int:
    ...
    # Associe la fenêtre à son entrée de menu : sans ça, les bureaux Wayland
    # affichent l'icône générique de Qt dans la barre des tâches.
    app.setDesktopFileName("chess-review")
